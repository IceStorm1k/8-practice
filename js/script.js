...
let population = document.createElement("button");
let languages = document.createElement("button");
let remotePanel = document.createElement("div");
...
// ! добавления классов css для всех элементов

container.classList.add("container");
container2.classList.add("container2");
container3.classList.add("container3");

headerSub.classList.add("header-sub");
remotePanel.classList.add("remote-panel");
population.classList.add("btnStyle");
languages.classList.add("btnStyle");
mainSub.classList.add("main-sub");
charBlock.classList.add("char-block");

// ! создания html сайта
headerTitle.textContent = "World Countries Data";
headerSub.textContent = `Currently, we have ${countries_data.length} countries`;
population.textContent = "Population";
languages.textContent = "Languages";
mainSub.textContent = "10 Most Spoken languages in the world";

// ! добавление элементов на страницу
document.body.append(header);
header.append(container);
container.append(headerTitle);
container.append(headerSub);
document.body.append(main);

main.append(remotePanel);
remotePanel.append(container2);
container2.append(population);
container2.append(languages);
container2.append(mainSub);

// ! динамический блок с информацией о стране

main.append(charBlock);
charBlock.append(container3);

// ! добавление событий на кнопки population
population.addEventListener("click", () => {
	container3.innerHTML = "";
  
		 const world = document.createElement("div");
		 let protcent2 = document.createElement("div");
		 protcent2.classList.add("protcent");
		 let allValue = document.createElement("p") ;
		 allValue.classList.add("countryName");
			  world.classList.add("countryName");
			  world.textContent = "world";
			  container3.append(world);
			
			  let allPopulation = countries_data.reduce((acc, curr) => {
			  ...
			  protcent2.style.width = `${(allPopulation / 7758589152) * 100}%`;
			 
			  allValue.textContent = allPopulation;
			  container3.append(protcent2);
			  container3.append(allValue);
		 ...
		 .forEach((country) => {
			  ...
			  countryPopulation.classList.add("countryName");
			 
		  

			  let protcent = document.createElement("div");
			  protcent.classList.add("protcent");
			  const selectedValue = country.population;
			  ...
			  protcent.style.width = `${(selectedValue / 1800000000) * 100}%`;
			 
			  countryName.textContent = country.name;

			  countryPopulation.textContent = country.population;
			 
			  container3.append(countryName);
			  container3.append(protcent);
			  container3.append(countryPopulation);
		 });
});
languages.addEventListener("click", () => {
	container3.innerHTML = "";
	let count = countries_data.reduce((acc, curr) => {
		acc[curr.languages] = (acc[curr.languages] || 0) + 1;
		return acc;
	}, {});

	let sortedCount = Object.fromEntries(Object.entries(count).sort((a, b) => b[1] - a[1]));
	let languagesArr = Object.keys(sortedCount);
	languagesArr = languagesArr.slice(0, 10);

	languagesArr.forEach((lang) => {
		let langName = document.createElement("p");
		let langCount = document.createElement("p");
		langName.classList.add("countryName");
		langCount.classList.add("countryName");
		let protcent = document.createElement("div");
		protcent.classList.add("protcent");
		let selectedValue = sortedCount[lang]
		protcent.style.width = `${(selectedValue / 100) * 100}%`;
		langName.textContent = lang;
		langCount.textContent = sortedCount[lang];
		container3.append(langName);
		container3.append(protcent);
		container3.append(langCount);
	});
  });